# Dotfiles-hyprland
This repository stores the current version of my Linux desktop dotfiles from ~/.config.

# Installation
```
sudo pacman -Syu hyprland waybar wofi kitty wl-clipboard grim slurp mako hyprpaper swaync nwg-look fastfetch zsh kate
```

# What goes where
- Replace /home/use with your actual home path if different.
```
# copy configuration folders to ~/.config (creates hypr, waybar, wofi, etc..)
mkdir -p /home/use/.config
cp -r hypr /home/use/.config/hypr
cp -r fastfetch /home/use/.config/fastfetch
cp -r swaync /home/use/.config/swaync
cp -r waybar /home/use/.config/waybar
cp -r wofi /home/use/.config/wofi
```

# Scrennshot
<img width="1920" height="1080" alt="screenshot-20260606-163902" src="https://github.com/user-attachments/assets/72f74aeb-be04-4bb5-a866-01694b87be5d" />
Last updated: 2026-7-30
